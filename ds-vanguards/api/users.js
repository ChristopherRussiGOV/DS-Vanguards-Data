...
module.exports = async (req, res) => {
  ...
  // Moderador+ pode visualizar, admin pode editar
  ...
  // Edição/exclusão só para admin
  if (!requireRole(caller, 'admin')) return json(res, 403, { error: 'Sem permissão. Requer Admin.' });
  ...
  if (req.method === 'PUT') {
    ...
    try {
      ...
      if (role) {
        ...
        await logAction(caller, 'Alterou cargo de usuário', `Usuário: ${target.rows[0].username} → ${role}`);
      }
      if (password) {
        ...
        await logAction(caller, 'Alterou senha de usuário', `Usuário: ${target.rows[0].username}`);
      }
      if (username) {
        ...
        await logAction(caller, 'Renomeou usuário', `${target.rows[0].username} → ${username}`);
      }
  ...
  if (req.method === 'DELETE') {
    ...
    try {
      ...
      await logAction(caller, 'Excluiu usuário', `Usuário: ${target.rows[0].username}`);
